...
func (f Telephone) Info() string {
	return fmt.Sprintf("Модель: %s, Вес модели: %s, Размер дисплея модели: %d минут", f.Model, f.Weight, f.display_size)
}
func (f *Telephone) UpdateModel(newModel string) {
	f.Model = newModel
}
func (f Telephone) Buy() {
	fmt.Printf("Вы купили телефон модели:%s веса:%s у которого размер дисплея:%s", f.Model, f.Weight, f.display_size)
}
func ExecuteLab6() {
